...
class ComprehensiveTaxEngine:
  ...
  def run_single_year(
    ...
    is_eligible = (property_type == "RESIDENT_1HOME") and (not is_joint_default) and is_taxable
  ...
  for idx, res in enumerate(results):
    with cols[idx]:
      ...
      with st.container(border=True):
          ...
          # [수정] 종부세 0원 이유를 심플하고 직관적으로 렌더링
          if res.get('is_saved_by_threshold', False):
              st.success("✅ **종부세 0원**")
              st.markdown(f"└ 과세기준 ({res['tax_threshold'] // 100000000}억) 이하")
          elif res['tax_base'] <= 0:
              st.success("✅ **종부세 0원**")
              st.markdown(f"└ 기본공제 ({res['deduction'] // 100000000}억) 이하")
